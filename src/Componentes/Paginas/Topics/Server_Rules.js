...
function Server_Rules() {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        const response = await fetch('https://sheetdb.io/api/v1/7jsc9tsp9z10i/search?Page=Server_Rules');
        if (!response.ok) {
  ...
  return (
    <>
      ...
      {Object.entries(groupedComments).map(([title, comments]) => (
        <Comment key={title} title={title} page="Server_Rules" comments={comments} />
      ))}
    </>
  );
}
